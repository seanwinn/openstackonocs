...
echo "grub-pc	grub-pc/install_devices	multiselect	/dev/vda" | sudo debconf-set-selections
...
#glance image-create --name='Ubuntu 12.04 x86_64 Server' --disk-format=qcow2 --container-format=bare --public --file /home/ubuntu/precise-server-cloudimg-amd64-disk1.img
glance image-create --name='Mediawiki-Web' --disk-format=qcow2 --container-format=bare --public --file /home/ubuntu/cirros-0.3.0-x86_64-disk.img
glance image-create --name='Mediawiki-DB' --disk-format=qcow2 --container-format=bare --public --file /home/ubuntu/cirros-0.3.0-x86_64-disk.img
#RAMDISK_ID=$(glance image-create --container-format ari --disk-format ari --is-public true --name mediawiki-ari --file /home/ubuntu/turnkey-mediawiki-13.0-wheezy-amd64-initrd | awk '/\ id \ / {print $4}')
#KERNEL_ID=$(glance image-create --container-format aki --disk-format aki --is-public true --name mediawiki-aki --file /home/ubuntu/turnkey-mediawiki-13.0-wheezy-amd64-kernel | awk '/\ id \ / {print $4}')
#glance image-create --name mediawiki --disk-format ami --container-format ami --is-public true --file /home/ubuntu/turnkey-mediawiki-13.0-wheezy-amd64.img --property kernel_id=$KERNEL_ID --property ramdisk_id=$RAMDISK_ID
###############################
# Neutron Install
###############################
# Create database
...
sudo apt-get -y --force-yes install neutron-dhcp-agent neutron-l3-agent neutron-plugin-openvswitch neutron-plugin-openvswitch-agent neutron-lbaas-agent
...
sudo sed -i "s/# service_plugins =/service_plugins = neutron.services.loadbalancer.plugin.LoadBalancerPlugin/g" /etc/neutron/neutron.conf

echo "
device_driver = neutron.services.loadbalancer.drivers.haproxy.namespace_driver.HaproxyNSDriver
interface_driver = neutron.agent.linux.interface.OVSInterfaceDriver" | sudo tee -a /etc/neutron/lbaas_agent.ini
...
sudo apt-get -y --force-yes install rabbitmq-server nova-novncproxy novnc nova-api nova-ajax-console-proxy nova-cert nova-conductor nova-consoleauth nova-doc nova-scheduler python-novaclient libvirt-bin pm-utils sysfsutils nova-compute-qemu nova-doc
...
sudo apt-get install -y --force-yes xfsprogs qemu-utils cinder-api cinder-scheduler cinder-volume open-iscsi python-cinderclient tgt sysfsutils
...
sudo sed -i "s/    'enable_lb': False/    'enable_lb': True/g" /etc/openstack-dashboard/local_settings.py
...
/home/ubuntu/create_neutron_networks
